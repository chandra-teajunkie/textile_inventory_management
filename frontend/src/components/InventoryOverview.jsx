...
function InventoryOverview({ toast }) {
  ...
  const handleAddItem = async () => {
    // Validate form - FIXED: Added color to validation
    if (!newItem.material || !newItem.material_type || !newItem.storage_location || !newItem.color) {
      toast.current.show({
        severity: "warn",
        summary: "Validation Error",
        detail: "Please fill all required fields (Material, Type, Color, Storage Location)",
        life: 3000,
      })
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        console.error("Inventory creation failed:", responseData)
        toast.current.show({
          severity: "error",
          summary: "Error",
          detail: responseData.detail || responseData.message || `Create failed: ${res.status}`,
          life: 3000,
        })
      ...
      const mapped = {
        id: responseData.inventory_id || responseData.id,
        storage_location: responseData.storage_location || "",
        material_type: responseData.material_type || "",
        color: responseData.color || "",
        quantity_weight: responseData.quantity_weight ?? 0,
        quantity_bags: responseData.quantity_bags ?? null,
        weight_per_bag: responseData.weight_per_bag ?? null,
      ...
      console.error("Network error creating inventory:", err)
      toast.current.show({
        severity: "error",
        summary: "Network Error",
        detail: err.message || "Failed to create inventory item",
        life: 3000,
      })
